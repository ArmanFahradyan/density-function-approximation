...
def gamma_pdf(x, alpha=0.5, lambda_=0.5):
    return gamma.pdf(x, alpha, scale=lambda_)
...
def uniform_generator(n, a=-3.0, b=3.0):
    return np.random.uniform(a, b, n)


def gamma_generator(n, alpha=0.5, lambda_=0.5):
    return np.random.gamma(alpha, lambda_, size=n)
